...
def display_sentiment_stats(processed_chat_stream):
    """Display the number of messages per sentiment."""
...
def display_threads(processed_chat_stream):
    """Display the different threads and their messages."""
    st.markdown("#### Threads")
    st.markdown("Messages clustered by their topic")
    for thread_ix, (_, messages) in enumerate(
        groupby(processed_chat_stream, lambda x: x.root_message_id)
    ):
        with st.expander(f"🧵 Thread {thread_ix}"):
            for message in messages:
                st.markdown(f"[{message.user_id}] " f"  {message.text}")
...
def display_messages_by_intent(intent_to_messages):
    """Display chat messages by intent in tab-separated containers."""
    tabs = st.tabs(
        [
            f"{props['title']} ({len(intent_to_messages[intent])})"
            for intent, props in INTERESTING_INTENTS.items()
        ]
    )
    for tab, props, messages in zip(
        tabs,
        INTERESTING_INTENTS.values(),
